	...
	<tbody>
		<c:forEach items="${pageBean.list}" var="service">
			<tr>
				...
				<td>
					<a href="${pageContext.request.contextPath}/serviceManager_feedBackingService.action?serv_no=${service.serv_no}">处理</a>
				</td>
			</tr>
		</c:forEach>
	</tbody>
...
<div>
	...
	<c:if test="${pageBean.currentPage != 1}">
		【<a href="${pageContext.request.contextPath }/serviceManager_feedBackService?currentPage=${pageBean.currentPage-1 }">前一页</a>】
	</c:if>
	
	<c:if test="${pageBean.currentPage != pageBean.totalPage}">
		【<a href="${pageContext.request.contextPath }/serviceManager_feedBackService?currentPage=${pageBean.currentPage+1 }">后一页</a>】
	</c:if>
</div>
</body>
</html>