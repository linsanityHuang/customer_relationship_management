	...
	<tbody>
		<c:forEach items="${pageBean.list}" var="service">
			<tr>
				...
				<td>
					<c:if test="${serviceManager.serv_status == 5}">
						已归档
					</c:if>
				</td>
				<td>
					<a href="${pageContext.request.contextPath}/serviceManager_showFileService.action?serv_no=${service.serv_no}">查看</a>
				</td>
			</tr>
		</c:forEach>
	</tbody>
...
<div>
	...
	<c:if test="${pageBean.currentPage != 1}">
		【<a href="${pageContext.request.contextPath }/serviceManager_fileService?currentPage=${pageBean.currentPage-1 }">前一页</a>】
	</c:if>
	
	<c:if test="${pageBean.currentPage != pageBean.totalPage}">
		【<a href="${pageContext.request.contextPath }/serviceManager_fileService?currentPage=${pageBean.currentPage+1 }">后一页</a>】
	</c:if>
</div>
</body>
</html>