...
<form action="${pageContext.request.contextPath}/dictionary_multiQuery.action" method="post">
	<a></a>
	<a href="${pageContext.request.contextPath}/dictionary_addCustLevel.action">新建</a>
		...
		<tbody>
			<c:forEach items="${pageBean.list}" var="dictionary">
				<tr>
					...
					<td>
						<a href="${pageContext.request.contextPath}/dictionary_editLevel.action?basi_no=${dictionary.basi_no}">编辑</a>
						<a href="${pageContext.request.contextPath}/dictionary_deleteLevel.action?basi_no=${dictionary.basi_no}" onclick="delcfm()">删除</a>
					</td>
				</tr>
			</c:forEach>
		</tbody>
	...
	<div>
		...
		<c:if test="${pageBean.currentPage != 1}">
			【<a href="${pageContext.request.contextPath }/dictionary_showCustLevel?currentPage=${pageBean.currentPage-1 }">前一页</a>】
		</c:if>
		
		<c:if test="${pageBean.currentPage != pageBean.totalPage}">
			【<a href="${pageContext.request.contextPath }/dictionary_showCustLevel?currentPage=${pageBean.currentPage+1 }">后一页</a>】
		</c:if>
	</div>
</body>
</html>